...
import type { Truck, Driver, Carrier, ScheduleEntry } from '@/lib/types';
import { format }
from 'date-fns';
...
const initialTrucks: Truck[] = [
  { id: 'truck1', name: 'Alpha Hauler', licensePlate: 'TRK-001', model: 'Volvo VNL', year: 2022, carrierId: 'carrier1', driverId: 'driver1', maintenanceStatus: 'Good' },
  { id: 'truck2', name: 'Beta Mover', licensePlate: 'TRK-002', model: 'Freightliner Cascadia', year: 2021, carrierId: 'carrier2', driverId: 'driver2', maintenanceStatus: 'Needs Service' },
  { id: 'truck3', name: 'Gamma Transporter', licensePlate: 'TRK-003', model: 'Peterbilt 579', year: 2023, carrierId: 'carrier1', maintenanceStatus: 'In Service' },
];
...
const initialCarriers: Carrier[] = [
  { id: 'carrier1', name: 'Speedy Logistics', contactPerson: 'Mike Ross', contactEmail: 'mike.ross@speedylog.com', contactPhone: '555-0001', contractDetails: 'Primary Carrier Agreement, expires 2025-12-31' },
  { id: 'carrier2', name: 'Reliable Transport Inc.', contactPerson: 'Sarah Connor', contactEmail: 's.connor@reliabletransport.com', contactPhone: '555-0002', contractDetails: 'Secondary Carrier, flexible terms' },
];
...
export function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  const removeTruck = useCallback((truckId: string) => {
    ...
    setScheduleEntries(prev => prev.filter(s => s.truckId !== truckId)); // Also remove associated schedules
  }, []);
  ...
  const removeDriver = useCallback((driverId: string) => {
    ...
    // Unassign driver from trucks and schedules
  ...
  const removeCarrier = useCallback((carrierId: string) => {
    ...
    // Potentially handle trucks associated with this carrier (e.g., unassign or warn)
    // For simplicity, we'll just remove the carrier. Trucks would need carrierId to be optional or reassigned.
  }, []);
